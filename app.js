...
(function () {
  ...
  // Tile pyramid: each sector's content is sliced into PT_W x PT_H tiles at several resolution levels;
  // the detail layer loads only the on-screen tiles at the level matching the current zoom.
  ...
  // LRU cache of decoded detail tiles. At 6400×3200 each tile decodes to ~82 MB, so panning a big map
  // can pile up decoded bitmaps faster than GC frees them -> the browser slows then hangs. Two fixes:
  //  (a) decode via createImageBitmap — async and OFF the main thread, so panning never blocks on a
  //      decode (tiles just stream in), and
  //  (b) close() a tile's bitmap the instant it's evicted, freeing the memory deterministically rather
  //      than waiting for GC.
  // Budget ~512 MB of decoded bitmaps, sized by the PYRAMID TILE (not the whole sector). At 1600×800
  // (~5 MB/tile) that's ~100 tiles — far more than the ~10-20 ever on screen, so revisits are instant
  // while memory stays bounded (deterministic close() on eviction). Don't prefetch off-screen tiles.
  ...
  const topToggles = [['grid', 't-grid'], ['detail', 't-detail'], ['roofs', 't-roofs'], ['bigmap', 't-bigmap']];
  ...
  function draw() {
    ...
    if (layers.bigmap && live) drawBigmaps();
  ...
  // Bigmap coverage: tint each live sector by whether its map was upgraded to a 360×360 "bigmap"
  // (green) or still renders from a standard 160×160 vanilla map (orange). `s.big` comes from the
  // manifest (set at build time from the map's dimensions). Works at every level (cur.sectors).
  function drawBigmaps() {
    for (const s of cur.sectors) {
      const c = outline(s.sx, s.sy).map((p) => toScreen(p.x, p.y)); // [TL, TR, BR, BL]
      ctx.fillStyle = (s.big ? BIG_COL : STD_COL) + '55';
      ctx.beginPath(); ctx.moveTo(c[0].x, c[0].y); for (let i = 1; i < 4; i++) ctx.lineTo(c[i].x, c[i].y); ctx.closePath(); ctx.fill();
    }
  }
  ...
  for (const k of ['grid', 'detail', 'roofs', 'cool', 'town', 'mine', 'sam', 'heli', 'ship', 'garrison', 'patrol', 'poi', 'npc', 'loot', 'creatures', 'roads', 'terrain', 'bigmap']) {
