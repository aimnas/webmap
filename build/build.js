...
const TILE_W = 7200, TILE_H = 3600;  // exactly HALF the native iso render (14400x7200) — a clean 1:2 (2x)
                                     // downsample of the source, sharper than the prior odd 2.25x->6400.
// TILE PYRAMID: the warped content is sliced into PT_W x PT_H tiles at several resolution levels, so
// the viewer loads only the on-screen tiles at the current zoom — small, fast decodes — instead of one
// huge whole-sector image (which queued up decodes and stalled panning). z0 = 1/4 res (1 tile), up to
// full res (NxN). Tile world-coverage = cellW/cols x cellH/rows. PT_W must divide all level widths:
// 7200/4=1800, 7200/2=3600, 7200 -> 1800 divides each.
  ...
  for (const lvl of LEVELS) {
    ...
    for (const code of codes) {
      ...
      for (const roofPass of passes) {
        ...
        else { sector.tiles = code; sector.big = r.dat.cols > 200; } // big = upgraded 360² bigmap (vs 160² vanilla)
      }
